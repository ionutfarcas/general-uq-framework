...
class DimensionPriority(DimensionAdaptivity):
	...
	def do_one_adaption_step_preproc(self):
		...
		for local_dir_var in self._local_dir_var_max.keys():
			...
			print 'INDEX MAX', np.flipud(np.argsort(self._local_dir_var_max.keys()))
	...
	def check_termination_criterion(self):
		...
		if len(self._E.values()) == 0 or max_level >= self._max_level or self._curr_max_dir_var <= self._tol:
