...
class DimensionPriority(DimensionAdaptivity):
	...
	def get_edge_set_dna(self):
		...
		local_dir_vars 	= np.array([local_dir_var for local_dir_var in self._local_dir_var.values()])
	...
	def do_one_adaption_step_preproc(self):
		...
		neighbors_edge_set = Multiindex(self._dim).get_successors_edge_set(self._multiindex_set, self._E.values())
		...
		edge_set 							= [multiindex.tolist() for multiindex in self._E.values()]
		existing_keys 						= []
		self._multiindices_edge_set_to_del 	= []
	...
	def check_termination_criterion(self):
		...
		if len(self._E.values()) == 0 or max_level >= self._max_level or np.sum(self._edge_set_dna) == 0:
